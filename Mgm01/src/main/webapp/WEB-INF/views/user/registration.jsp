...
<title>회원가입</title>
    ...
    function checkPass() {
   		if ($('#password').val().length < 4) {
   			$('#chk_pass').empty();
   			$('#chk_pass').append("<span>비밀번호는 4글자 이상이여야 합니다</span>");
   			cor_pass=false
   		} else{
	   		$('#chk_pass').empty();
   			cor_pass=false;
    ...
    function checkRepass(){
   		if($('#repassword').val()!=$('#password').val()){
   			$('#chk_repass').empty();
	   		$('#chk_repass').append("<span>비밀번호가 일치하지 않습니다.</span>");
	   		cor_pass=false;
   		} else{
   			$('#chk_repass').empty();
   			$('#chk_repass').append("<span>비밀번호가 일치합니다.</span>");
   			cor_pass=true;
   		}
    }
    ...
    function checkBpass() {
   		if ($('#b_password').val().length < 4) {
   			$('#chk_bpass').empty();
   			$('#chk_bpass').append("<span>환전비밀번호는 4글자여야합니다.</span>");
   			cor_bpass=false
   		} else{
	   		$('#chk_bpass').empty();
   			cor_bpass=true;
...
function checkId(type) {
	if(type=="id"){
    	if ($('#id').val().length < 4) {
    		$('#chk_id').empty();
   			$('#chk_id').append("<span>아이디는 4글자 이상이어야 합니다.</span>");
   			cor_id = false;
    	} else{
    		$('#chk_id').empty();
    		...
    		$.ajax({
				...
				if(msg.length<14){
    				$('#chk_id').append("<span style=\"color:blue;\">"+msg+"</span>");
    				cor_id = true;
				}
				else
					$('#chk_id').append("<span style=\"color:red;\">"+msg+"</span>");
    		});
    	} 
	}
	...
	if(type=="recmd_id"){
    		$.ajax({
				...
				$('#chk_recmdId').empty();
				if(msg.length<18){
    				$('#chk_recmdId').append("<span style=\"color:blue;\">"+msg+"</span>");
    				cor_id = true;
				}
				else
					$('#chk_recmdId').append("<span style=\"color:red;\">"+msg+"</span>");
    		});
	}
}
    </script>
...
<!--                 </span> -->
      ...
      <div  class="form">
    		<form id="contactform" action="register" method="POST"> 
    			<p class="contact"><label for="id">아이디</label></p> <div id="chk_id"></div>
    			<input id="id" name="id" placeholder="아이디를 입력하세요" required="required" type="text" onblur="checkId('id')"/> 
    			
 				<p class="contact"><label for="password">패스워드</label></p> <div id="chk_pass"></div>
    			<input type="password" id="password" name="password" required="required" onkeyup="checkPass()" /> 
                <p class="contact"><label for="repassword">패스워드 재입력</label></p> <div id="chk_repass"></div>
    			...
    			<p class="contact"><label for="password">환전비밀번호</label></p><div id="chk_bpass"></div> 
    			<input type="password" id="b_password" name="b_password" required="required" maxlength="4" onkeyup="checkBpass()" pattern="{0-9}{4}"/> 
    			 
    			<p class="contact"><label for="id">추천인 아이디</label></p> <div id="chk_recmdId"></div>
    			<input id="recmd_id" name="recmd_id" placeholder="추천인 아이디를 입력하세요" required="required" type="text" onblur="checkId('recmd_id')"/>  
				<br><br>
            
            
            	<input class="button" name="submit" id="submit" tabindex="5" value="회원가입" type="submit"> 	 
  		 </form> 
	</div>      
</div>
